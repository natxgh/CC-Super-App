...
//
// ⚠️  IMPORTANT: หลังจาก PO กรอก Answer แล้ว → ห้าม PUT record นั้นอีก
//     เพราะ Lark Base จะ update "Modified By" → ชื่อ PO จะหายไป
//     ใช้ safe-update.mjs ทุกครั้งที่ต้องแก้ไข records ใน PO table
...
if (featureField?.type === 3 || featureField?.type === 4) {
  // ✅ keep existing options ทั้ง object (id + color + name) — ห้าม map เป็นแค่ name
  // ถ้า drop id ทิ้ง → Lark ออก ID ใหม่ทั้งหมด → records เก่าที่ผูก ID เดิมหลุดหมด
...
// อ่าน pending.json เดิม (รองรับทั้ง array และ object format เดิม)
const pendingPath = path.join(STATE_DIR, 'pending.json');
let batches = [];
if (fs.existsSync(pendingPath)) {
  const existing = JSON.parse(fs.readFileSync(pendingPath, 'utf8'));
  batches = Array.isArray(existing) ? existing : [existing];
  // ลบ batch เดิมของ feature นี้ออก (จะถูก replace ด้วยอันใหม่)
  batches = batches.filter((b) => b.feature !== feature);
}
...
fs.writeFileSync(pendingPath, JSON.stringify(batches, null, 2));
