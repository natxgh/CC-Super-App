...
// ดึง "Feature::TC No." ที่มีอยู่แล้ว (เพื่อ skip duplicate — key ต้องรวม Feature ด้วยเพราะ TC No. ซ้ำข้าม feature ได้)
let existingTCs = new Set();
try {
  let pageToken = '';
  do {
    ...
    (res.data?.items||[]).forEach(r => {
      ...
      const feat = r.fields?.['Feature'];
      const tcStr   = tc   ? (typeof tc   === 'object' ? tc.value   : String(tc))   : '';
      const featStr = feat ? (typeof feat === 'object' ? feat.value : String(feat)) : '';
      if (tcStr) existingTCs.add(`${featStr}::${tcStr}`);
    });
    pageToken = res.data?.has_more ? res.data.page_token : '';
  } while (pageToken);
  console.log(`→ มี ${existingTCs.size} TC อยู่แล้วใน Base (จะ skip)`);
} catch(e) {
  console.warn('⚠️ ดึง existing records ไม่ได้:', e.message);
}

// แบ่ง batch 10 แถว
const toUpload = rows.filter(r => !existingTCs.has(`${r['Feature']}::${r['TC No.']}`));
