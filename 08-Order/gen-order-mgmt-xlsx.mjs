...
const SRC = {
  ...
  'VO-detail':{ cat:'POSITIVE', title:'Order Detail page renders all elements',
    arrange:'API seed: seedOrder(SEED_ORDER_OOS) — create a fresh order (iPhone 17 Pro Screen item) via GraphQL CreateOrder; open via the seeded orderId (no pre-existing data). Teardown: CancelOrder.',
    steps:'Open the order detail (click the row)',
    data:'Order: <seeded orderId> · item iPhone 17 Pro Screen',
    expected:'Shows Order No + status badge + date + requester + Print button + Bill card (Bill To/Billing Address/Ship To/Shipping Address/Ship By) + ORDER ITEMS + OPERATING PROCEDURE (9-step workflow)' },
  'VO-stock':{ cat:'POSITIVE', title:'Order Item shows Out of Stock badge',
    arrange:'Same seeded order as VO-detail (iPhone 17 Pro Screen item). ENV_DEPENDENT: the Out of Stock badge appears only if that product’s stock = 0 in the QA env.',
    steps:'Open detail → view the ORDER ITEMS section',
    data:'Item: iPhone 17 Pro Screen',
    expected:'Item "iPhone 17 Pro Screen" shows an "Out of Stock" badge (red) + Quantity + Price (env-dependent on stock = 0)' },
  'VO-chat0':{ cat:'POSITIVE', title:'Chat box empty state = No Comment',
    arrange:'Same seeded order as VO-detail (fresh = no comments yet)',
  ...
  'VO-chat1':{ cat:'POSITIVE', title:'Add a Comment to an order',
    arrange:'Same seeded order as VO-detail · Chat box open',
  ...
  'UD-bill': { cat:'POSITIVE', title:'Edit Bill/Shipping inline then Save (Create Order)',
    arrange:'API seed: seedOrder(SEED_ORDER) — fresh order in Create Order state (OS000); open via the seeded orderId. Teardown: CancelOrder.',
  ...
  'UD-item': { cat:'POSITIVE', title:'Edit Order Item quantity (Create Order)',
    arrange:'Same seeded order as UD-bill (Create Order state)',
  ...
  'UD-title':{ cat:'POSITIVE', title:'Edit the order Title',
    arrange:'Same seeded order as UD-bill',
  ...
  'UD-lock': { cat:'NEGATIVE', title:'After Submit — Bill & Items are locked',
    arrange:'API seed: seedOrder(SEED_ORDER) → advanceOrder(OS001) → advanceOrder(OS003) — fresh order advanced to Request Approved; open via the seeded orderId (no pre-existing data). Teardown: CancelOrder.',
    steps:'Open detail → look for the pencil on the Bill card and ORDER ITEMS',
    data:'Order: <seeded orderId> (Request Approved, OS003)',
  ...
  'WF-adv1': { cat:'POSITIVE', title:'Advance workflow: คำสั่งซื้อ → ส่งคำขอ',
    arrange:'The order created in the same TS-01 flow (Create Order state). API alt: seedOrder(SEED_ORDER). Logged in as PIC of the "ส่งคำขอ" (Request) step. Teardown: CancelOrder (TS-01 walks to Complete OS009 which is terminal — see FIXME-PLAN no-DeleteOrder gap).',
  ...
  'WF-pic':  { cat:'NEGATIVE', title:'Non-PIC user → Advance button does not appear',
    arrange:'API seed: seedOrder(SEED_ORDER) — fresh order; then log in with a non-PIC account (ORD_NON_PIC_USERNAME/PASSWORD; PIC roles = Warehouse Approver / Manager). Teardown: CancelOrder.',
  ...
  'WF-sla':  { cat:'POSITIVE', title:'Current step over SLA → Overdue badge',
    arrange:'ENV_DEPENDENT: needs an order at OS003 (ได้รับการอนุมัติ) idle > 61 min — cannot seed + wait 61 min in CI. Provide ORD_OVERDUE_ID env pointing at such an order, else the step is skipped.',
  ...
  'WF-notify':{ cat:'POSITIVE', title:'Event Notification on workflow Advance',
    arrange:'The order from the TS-01 flow (has a requester + next-step PIC). API alt: seedOrder(SEED_ORDER).',
    steps:'PIC taps the Advance button of the current step',
    data:'Action: advance step (the seeded/created order)',
  ...
  'CO-cancel':{ cat:'NEGATIVE', title:'Cancel an order before Approved',
    arrange:'API seed: seedOrder(SEED_ORDER) — fresh order in Create Order state (before Approved); open via the seeded orderId. (Cancel itself is the teardown — order ends in Cancel state.)',
    steps:'Open detail → tap the "Cancel" button (yellow) → confirm in the dialog',
    data:'Order: <seeded orderId>, status Create Order',
  ...
  'CO-canapr':{ cat:'NEGATIVE', title:'Cancel button at the Approved step should be blocked (BUG)',
    arrange:'Same seeded order as UD-lock (seedOrder(SEED_ORDER) → advanceOrder OS001 → OS003 = Request Approved). No pre-existing data. Teardown: CancelOrder.',
  ...
  'TL-sid':  { cat:'NEGATIVE', title:'Search by Order ID (BUG: does not filter)',
    arrange:'API seed: 2 orders — seedOrder(SEED_ORDER) + seedOrder(SEED_ORDER_OOS) — so a specific seeded orderId exists to search for (no hard-coded pre-existing ID). Teardown: CancelOrder.',
    steps:'Type the seeded Order ID in the Search box → tap Search',
    data:'<seeded orderId>',
    expected:'Expected: list filters to only the searched Order ID (Search should filter by Order ID per placeholder "Search request ID or part..."). Actual: returns all orders (no filtering) → expected to FAIL → open a defect. [PO ORD-Q7]' },
  'TL-spart':{ cat:'NEGATIVE', title:'Search by part name (BUG: does not filter)',
    arrange:'Same seeded orders as TL-sid (SEED_ORDER_OOS contains iPhone 17 Pro Screen) plus other orders without it',
  ...
  'TL-clear':{ cat:'POSITIVE', title:'Clear Filters restores the full list',
    arrange:'A search is active via the seeded orderId from VO-detail (Clear Filters button visible)',
