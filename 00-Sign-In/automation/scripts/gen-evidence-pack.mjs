...
// คอลัมน์ตรง sheet TestResult (ไฟล์ 2)
const COLS = [
  'Scenario No.', 'Scenario Name', 'Business Conditions', 'Arrange', 'TestcaseNo', 'Case Title Name',
  'Test category', 'Test Type', 'Test Steps', 'Expected Result', 'UI Design',
  'Test Result', 'Result Note', 'Remark', 'Test Date', 'Test By',
];
...
for (const r of data.results) {
  ...
  for (const st of steps) {
    ...
    lines.push(
      [scNo, scName, '', '', tcNo, caseTitle, cat, 'SYSTEMTEST', tSteps, '', '', result, note, '', testDate, testBy].map(csv).join(','),
    );
    records.push({
      scenarioNo: scNo, scenarioName: scName, testcaseNo: tcNo, caseTitle, testCategory: cat,
      testType: 'SYSTEMTEST', testSteps: tSteps, expectedResult: '', testResult: result,
      evidenceFile: eviFile, videoFile: scnVideo, remark: '', testDate, testBy,
    });
    rowCount++;
  }
}
...
fs.writeFileSync(
  ...
  `# Evidence Pack → sheet TestResult — ${data.finishedAt}

scenario: ✅ ${passed} · ❌ ${failed} · รวม ${data.results.length} · แถวราย TC ${rowCount} · ภาพ ${imgCount} · วิดีโอ ${vidCount}

## เก็บผลเข้า Lark Base (sheet TestResult)
คอลัมน์ใน \`lark-test-result-import.csv\` ตรง sheet **TestResult** — ID เป็นรูปแบบทีม (\`TS-01\` / \`TS-01_TC-01\`)
- **บน Lark Base:** คอลัมน์ design (Scenario Name / Business Conditions / Arrange / Test Steps / Expected / UI Design) เป็น **lookup จาก TestCases** → ที่นี่เว้นว่าง/ใส่เท่าที่รู้ได้
  สิ่งที่ automation เติมให้จริง = **Test Result · Result Note · Test Date · Test By** (Remark เว้นให้คนใส่)
- Import แล้วลาก \`evidence/<TC>.png\` ลง **Result Note** · วิดีโอทั้ง scenario ที่ \`videos/<Scenario>.webm\`
- ยิงอัตโนมัติเข้า Lark: \`npm run file:lark -- --confirm\` (เขียนเฉพาะ 5 คอลัมน์ผล ตาม TestcaseNo)
