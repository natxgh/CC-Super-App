...
const scenarios = [
  S('TS-01', 'Full case lifecycle E2E — create a case through to successful closure', '1. Contact Method options (CALL/METTLINK/METTRIQ/IOT-Alert/Other)\n2. Priority auto-set by Case Type\n3. Phone auto-links Customer Profile\n4. Required fields complete (Types, Contact Method, Case Details, Service Center)\n5. Attach photo jpg/png ≤ 1MB\n6. Submit → Confirm → case saved (New)\n7. Kanban grouping by status\n8. Lifecycle forward (New→Assigned→Acknowledged→En Route→On Site)\n9. Close requires Result + Result Details ≤ 1000\n10. Close approval flow → Completed', [
    ...
    ['TC-02','Priority auto-set from Case Type','P','On the Add New Case page','Select a Case Type','Types = "1002-Camera Malfunction -Repair" (Priority value = 3 per Case Configuration matrix)','Priority badge automatically shows "High Priority" (Red) — priority 3 falls in the 1–3 range per Q15 mapping confirmed 2026-06-25'],
  ...
  S('TS-05', 'Priority auto-set — verify all 6 distinct priority levels (matrix 2026-06-22)', '1. Priority auto-set by Case Type (AC3)\n2. Case Type matrix verified 2026-06-22: 11 device types / 27 sub-types / priority values 3,4,6,7,8,9\n3. Q15 resolved 2026-06-25: 0=Critical(Red) · 1–3=High(Red) · 4–6=Medium(Yellow) · 7–9=Low(Blue)', [
    ['TC-01','Priority 3 — Camera Malfunction Repair','P','Logged in as agent, on the Add New Case page (/cms/case/creation)','Select the Case Type dropdown and choose the sub-type','Types = "1002-Camera Malfunction -Repair" (Priority 3, SLA 97min, Workflow: BMA-Camera Flow)','Priority badge appears automatically showing "High Priority" (Red) — priority 3 falls in the 1–3 range'],
    ['TC-02','Priority 4 — Camera Malfunction Investigation','P','On the Add New Case page','Select the Case Type dropdown and choose the sub-type','Types = "1001-Camera Malfunction -Investigation" (Priority 4, SLA 97min, Workflow: BMA-Camera Flow)','Priority badge shows "Medium Priority" (Yellow) — priority 4 falls in the 4–6 range'],
    ['TC-03','Priority 6 — Camera Malfunction Service','P','On the Add New Case page','Select the Case Type dropdown and choose the sub-type','Types = "1003-Camera Malfunction -Service" (Priority 6, SLA 97min, Workflow: BMA-Camera Flow)','Priority badge shows "Medium Priority" (Yellow) — priority 6 is the upper boundary of the 4–6 range'],
    ['TC-04','Priority 7 — Water Monitoring Devices Investigation','P','On the Add New Case page','Select the Case Type dropdown and choose the sub-type','Types = "2001-Water Monitoring Devices -Investigation" (Priority 7, SLA 100min, Workflow: BMA-Water Monitoring Devices Flow)','Priority badge shows "Low Priority" (Blue) — priority 7 falls in the 7–9 range; SLA also changes to 100 min'],
    ['TC-05','Priority 8 — Water Monitoring Devices Maintenance','P','On the Add New Case page','Select the Case Type dropdown and choose the sub-type','Types = "2004-Water Monitoring Devices -Maintenance" (Priority 8, SLA 100min, Workflow: BMA-Water Monitoring Devices Flow)','Priority badge shows "Low Priority" (Blue) — priority 8 falls in the 7–9 range'],
    ['TC-06','Priority 9 — Water Monitoring Devices Outsourced','P','On the Add New Case page','Select the Case Type dropdown and choose the sub-type','Types = "2005-Water Monitoring Devices -Outsourced" (Priority 9, SLA 100min, Workflow: BMA-Water Monitoring Devices Flow)','Priority badge shows "Low Priority" (Blue) — priority 9 is the highest value in the current matrix and the upper boundary of the 7–9 range'],
  ]),
