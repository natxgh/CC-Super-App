// upload-product-inventory-results — upsert Test Result + Remark + video/screenshot into Lark Base
// composite key = Feature + TC No.
// DRY-RUN default · --confirm to write
// usage: node scripts/upload-product-inventory-results.mjs [--confirm]
import fs from 'fs';
...
const records = JSON.parse(fs.readFileSync(path.join(AUTO, 'test-results', 'product-inventory-records.json'), 'utf8'));
...
if (!CONFIRM) {
  console.log('=== DRY RUN — Product & Inventory results → Lark Base ===');
  ...
  for (const r of records) {
    ...
    console.log(`  [${hit}] ${icon} ${r.testcaseNo.padEnd(13)} ${r.testResult.padEnd(8)} files=[${evi.map((p) => (/\.webm$/.test(p) ? '🎬' : '🖼')).join('')}]${r.remark ? ' · ' + r.remark.slice(0, 60) : ''}`);
  }
  if (missing.length) console.log('\n⚠ MISSING (TC not in Base — upload design rows first):', missing.map((m) => m.testcaseNo).join(', '));
