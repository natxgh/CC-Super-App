// upload-appointment-results — upsert Test Result + Remark + video/screenshot into Lark Base
// composite key = Feature + TC No. (TC No. duplicates across features → ห้าม match TC No. เดี่ยว)
// DRY-RUN default · --confirm to write
// usage: node scripts/upload-appointment-results.mjs [--confirm]
import fs from 'fs';
...
const PROJ = path.resolve(__dirname, '..', '..');           // CC Super App/
...
const records = JSON.parse(fs.readFileSync(path.join(AUTO, 'test-results', 'appointment-records.json'), 'utf8'));
...
// composite map: `${Feature}||${TC No.}` → record_id
...
if (!CONFIRM) {
  console.log('=== DRY RUN — Customer Appointment results → Lark Base ===');
  ...
  for (const r of records) {
    ...
    const evi = [abs(r.videoFile), abs(r.evidenceFile)].filter((p) => p && fs.existsSync(p));
    console.log(`  [${hit}] ${r.testcaseNo.padEnd(12)} ${r.testResult.padEnd(8)} note=[${evi.map((p) => (/\.webm$/.test(p) ? '🎬' : '🖼')).join('')}] ${r.remark ? '· ' + r.remark.slice(0, 70) : ''}`);
  }
  if (missing.length) console.log('\nMISSING (Feature+TC No. ไม่พบใน Base — ต้อง upload design ก่อน):', missing.map((m) => m.testcaseNo).join(', '));
...
for (const r of records) {
  ...
  // Result Note attachment = video + screenshot
  const tokens = [];
  for (const p of [abs(r.videoFile), abs(r.evidenceFile)]) {
  ...
  if (res.code === 0) { updated++; console.log(`${r.testcaseNo} → ✅ ${r.testResult}${tokens.length ? ' +' + tokens.length + ' file' : ''}`); }
