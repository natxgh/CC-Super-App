// upload-case-results — upsert Test Result + Remark (+ video & screenshot into Result Note) into Lark Base
// composite key = Feature + TC No.  ·  Feature = "Case and Ticket Management"
// DRY-RUN default · --confirm to write
// usage: node scripts/upload-case-results.mjs [--confirm]
import fs from 'fs';
...
const records = JSON.parse(fs.readFileSync(path.join(AUTO, 'test-results', 'case-ticket-management-records.json'), 'utf8'));
const FEATURE = 'Case and Ticket Management';
const abs = (p) => (p ? path.resolve(AUTO, p) : null);
const evidenceOf = (r) => [r.videoFile, r.evidenceFile].map(abs).filter((p) => p && fs.existsSync(p));
...
if (!CONFIRM) {
  console.log('=== DRY RUN — Case and Ticket Management results → Lark Base ===');
  ...
  for (const r of records) {
    ...
    const evi = evidenceOf(r);
    const icon = r.testResult === 'PASSED' ? '✅' : r.testResult === 'FAILED' ? '❌' : r.testResult === 'BLOCKED' ? '🚧' : '⏸';
    console.log(`  [${hit}] ${icon} ${r.testcaseNo.padEnd(13)} ${r.testResult.padEnd(8)} files=[${evi.map((p) => (/\.webm$/i.test(p) ? '🎬' : '🖼')).join('')}]${r.remark ? ' · ' + r.remark.slice(0, 70) : ''}`);
  }
...
for (const r of records) {
  ...
  const tokens = [];
  for (const p of evidenceOf(r)) { const ft = await upload(p); if (ft) tokens.push({ file_token: ft }); }
